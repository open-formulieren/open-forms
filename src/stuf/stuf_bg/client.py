...
class Client(BaseClient):
    ...
    def get_values_for_attributes(
        self, bsn: str, attributes: list[str], template: str = ""
    ) -> bytes:
        context = {
        ...
        response = self.templated_request(
            "npsLv01",
            template=template if template else "stuf_bg/StufBgRequest.xml",
            context=context,
    ...
    def get_partners_or_children(
        ...
        if response_obj is not None:
            ...
            members_data: list[NaturalPersonDetails] = [
                NaturalPersonDetails(
                    bsn=member["gerelateerde"].get("inp.bsn"),
                    first_names=member["gerelateerde"].get("voornamen"),
                    initials=member["gerelateerde"].get("voorletters"),
                    affixes=member["gerelateerde"].get("voorvoegselGeslachtsnaam"),
                    lastname=member["gerelateerde"].get("geslachtsnaam"),
                    date_of_birth=(
