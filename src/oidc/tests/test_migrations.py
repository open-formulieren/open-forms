...
class OIDCConfigModelMigrationTests(TestMigrations):
    ...
    def test_migrate_digid_configuration_forward(self):
        ...
        self.assertEqual(new_digid_config.oidc_provider.oidc_op_discovery_endpoint, f"{BASE}/oidc/")
        self.assertEqual(new_digid_config.oidc_provider.oidc_op_jwks_endpoint, f"{BASE}/oidc/jwks")
        self.assertEqual(
            new_digid_config.oidc_provider.oidc_op_authorization_endpoint, f"{BASE}/oidc/auth"
        )
        self.assertEqual(new_digid_config.oidc_provider.oidc_op_token_endpoint, f"{BASE}/oidc/token")
        self.assertEqual(new_digid_config.oidc_provider.oidc_op_user_endpoint, f"{BASE}/oidc/user")
        ...
        self.assertEqual(new_digid_config.options["loa_settings"], {
            "claim_path": ["loa"],
            "default": "urn:etoegang:core:assurance-class:loa3",
            "value_mapping": ["loa_value"],
        })

        self.assertEqual(new_digid_config.options["identity_settings"], {
            "bsn_claim_path": ["bsn"],
        })
    ...
    def test_migrate_digid_machtigen_configuration_forward(self):
        ...
        self.assertEqual(new_config.options["identity_settings"],{
    ...
    def test_migrate_org_configuration_forward(self):
        ...
        self.assertEqual(new_config.options["user_settings"]["claim_mappings"],{
            "username": ["email"],
            "first_name": ["first_name"],
            "last_name": ["last_name"],
            "email": ["email"]
        ...
        self.assertEqual(new_config.options["groups_settings"], {
            "claim_mapping": ["roles"],
            "sync": True,
            "sync_pattern": "*",
