...
class V1Client(HALMixin, BRPClient):
    ...
    def _get_children(self, bsn: str) -> Children:
        ...
        children: Children = [
            NaturalPersonDetails(
                bsn=child.get("burgerservicenummer"),
                first_names=child.get("naam", {}).get("voornamen"),
                initials=child.get("naam", {}).get("voorletters"),
                affixes=child.get("naam", {}).get("voorvoegsel"),
    ...
    def _get_partners(self, bsn: str) -> Partners:
        ...
        partners: Partners = [
            NaturalPersonDetails(
                bsn=partner.get("burgerservicenummer"),
                first_names=partner.get("naam", {}).get("voornamen"),
                initials=partner.get("naam", {}).get("voorletters"),
                affixes=partner.get("naam", {}).get("voorvoegsel"),
                lastname=partner.get("naam", {}).get("geslachtsnaam"),
...
class V2Client(BRPClient):
    ...
    def find_persons(
        ...
        for person in persons:
            persons_mapping[person["burgerservicenummer"]] = person

        return persons_mapping
        ...
        for member in family_data:
            if "burgerservicenummer" in member:
                family_members_mappings[member["burgerservicenummer"]] = (
                    NaturalPersonDetails(
                        bsn=member.get("burgerservicenummer"),
                        first_names=member.get("naam", {}).get("voornamen"),
                        initials=member.get("naam", {}).get("voorletters"),
                        affixes=member.get("naam", {}).get("voorvoegsel"),
                        lastname=member.get("naam", {}).get("geslachtsnaam"),
