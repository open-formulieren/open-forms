...
class CommunicationPreferencesSerializer(
    ...
    def validate(self, attrs):
        profile_form_variable = attrs.get("profile_form_variable")
        form = self.context.get("form")

        if form and profile_form_variable:
            form_variable = FormVariable.objects.get(
                form=form, key=profile_form_variable
            )
            form_definition = FormDefinition.objects.get(
                pk=form_variable.form_definition.id
            )

            component = form_definition.configuration_wrapper[profile_form_variable]
