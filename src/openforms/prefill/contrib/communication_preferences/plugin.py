...
class ProfileCommunicationPreferences(BasePlugin[ProfileCommunicationPreferencesOptions]):
    ...
    def get_prefill_values_from_options(
        ...
        with get_customer_interactions_client(options["customer_interactions_api_group"]) as client:
    ...
    def check_config(self):
        for config in CustomerInteractionsAPIGroupConfig.objects.iterator():
            try:
                ...
                raise InvalidPluginConfiguration(
                    _("Client error: {exception}").format(
                        exception=exc
                    )
                ) from exc
