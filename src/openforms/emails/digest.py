...
class TileLayer:
    ...
    def _get_layer_names(self, root: type[_Element] | None, layer_type: OverlayType):
        match layer_type:
            case "wms":
    ...
    def _get_wms_layer_names(self, root: type[_Element] | None) -> list | None:
    ...
    def _get_wfs_layer_names(self, root: type[_Element] | None) -> list | None:
        # Try with common wfs standard namespace first (used in WFS 2.0)
        names = root.findall(
        ...
        if not names:
            ...
            names = root.findall(".//FeatureType/Name")
        return names
