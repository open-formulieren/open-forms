...
from openforms.contrib.haal_centraal.service import check_hc_config_for_partners
...
def collect_broken_configurations() -> list[BrokenConfiguration]:
    ...
    for config_name, exception_message in configurations.items():
        if exception_message:
            broken_configurations.append(
                BrokenConfiguration(
                    config_name=config_name, exception_message=exception_message
                )
            )
