...
const SessionStatusWrapper = ({
  showInModal = false,
  modalTitle,
  submitRowContent = null,
  onCloseModal,
  children,
}) => {
  const intl = useIntl();
  ...
  if (!showInModal) {
    return (
      <div className="session-status-wrapper session-status-wrapper--no-modal">{children}</div>
    );
  }
  ...
  return (
    ...
    >
      <div className="session-status-wrapper session-status-wrapper--modal">{children}</div>
      <SubmitRow isDefault>
        {submitRowContent ?? (
          <SubmitAction
            text={intl.formatMessage({
              description: 'Session status modal close button',
...
const useTriggerWarning = (numSeconds, timeToExpiryInMS) => {
  ...
  const [isReady, cancel, reset] = useTimeoutFn(() => setShowWarning(true), timeToWarningInMS);
  return [
    showWarning,
    () => {
...
const SessionStatus = () => {
  ...
  const className = classNames('session-status', {
  ...
  if (authFailure) {
    ...
    modalTitle = (
      <FormattedMessage
        description="Model title session expiry warning"
        defaultMessage="Session will expire"
      />
    );
    content = (
      <FormattedMessage
      ...
      <>
        <FormattedMessage
          ...
          values={{
            ...
            relativeTimeToExpiry: (
              <RelativeTimeToExpiry numSeconds={secondsToExpiry} key={date / 1000} raw />
            ),
