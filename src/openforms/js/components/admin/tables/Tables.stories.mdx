...
The `ChangelistTable` component uses the markup and styles of the standard Django
admin overview page for a given model/resource.
  ...
  >
    {({data, linkProp, rowKey, linkColumn}) => (
      <ChangelistTable
        ...
        data={data}
      >
        <ChangelistColumn objProp="driver"><>Driver</></ChangelistColumn>
        <ChangelistColumn objProp="team"><>Team</></ChangelistColumn>
      </ChangelistTable>
    )}
  </Story>
</Canvas>
...
By default, the first column is a clickable link. This can be disabled by setting
the `linkColumn` prop to `null`.
