...
const ChangelistTable = ({linkColumn = 0, linkProp = '', data = [], rowKey = '', children}) => {
  ...
  const headColumns = tableColumns.map((col, colIndex) => (
    <HeadColumn key={colIndex} content={col.header} />
  ));
...
const RowColumn = ({obj, index, objProp, linkColumn = 0, linkProp = '', isBool = false}) => {
  ...
  if (isBool) {
    content = value == null ? <IconUnknown /> : value ? <IconYes /> : <IconNo />;
  } else {
    content = value;
  }
  ...
  return <Component className={index === linkColumn ? 'nowrap' : ''}>{content}</Component>;
};
