...
const Today = ({name, value, onChange}) => {
  ...
  const intl = useIntl();
  const operatorChoices = Object.entries(OPERATORS).filter(([operator]) =>
    ['+', '-'].includes(operator)
  );
        ...
        <DSLEditorNode errors={null}>
          <NumberInput
            ...
            onChange={event => {
              const fakeEvent = {
                target: {
                  value: [parseInt(event.target.value, 10), ...rdelta.slice(1, 3)],
                },
              };
        ...
        <DSLEditorNode errors={null}>
          <NumberInput
            ...
            onChange={event => {
              const fakeEvent = {
                target: {
                  value: [rdelta[0], parseInt(event.target.value, 10), rdelta[2]],
                },
              };
              onChangeRelativeDelta(fakeEvent);
            }}
        ...
        <DSLEditorNode errors={null}>
          <NumberInput
            ...
            onChange={event => {
              const fakeEvent = {
                target: {
                  value: [...rdelta.slice(0, 2), parseInt(event.target.value, 10)],
                },
              };
              onChangeRelativeDelta(fakeEvent);
            }}
