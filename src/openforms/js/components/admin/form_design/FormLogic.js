...
const RuleBody = ({
  ...
  const {logicRuleAnalysisEnabled} = useContext(FormContext);

  // Case in which there is an error: a trigger step was specified, but this step cannot be found in the form
  if (!triggerFromStep && triggerFromStepIdentifier && !logicRuleAnalysisEnabled)
    setDisplayAdvancedOptions(true);
  displayAdvancedOptions = displayAdvancedOptions && !logicRuleAnalysisEnabled;
    ...
    <>
      <div className="logic-rule__rule">
        ...
        <div className="logic-trigger-container">
          ...
          {triggerFromStep && !logicRuleAnalysisEnabled && (
