...
const BackendOptionsFormRow = ({
    ...
    const hasOptionsForm = Boolean(
        backend && Object.keys(backend.schema.properties).length
    );
    ...
    const OptionsFormComponent =
        BACKEND_OPTIONS_FORMS[backend.id]?.form ?? FormRjsfWrapper;
...
const RegistrationFields = ({
    ...
    const backendChoices = backends.map((backend) => [
        backend.id,
        backend.label,
    ]);
