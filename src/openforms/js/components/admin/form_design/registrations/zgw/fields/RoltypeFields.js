...
const PartnerRoltypeSelect = ({roltypes, loading, zgwApiGroup, caseTypeIdentification}) => {
  const [, , fieldHelpers] = useField('partnerRoltype');
  const {setValue} = fieldHelpers;
...
PartnerRoltypeSelect.propTypes = {
  catalogueUrl: PropTypes.string,
};
    ...
    <>
      ...
      <FormRow>
        <Field
          ...
          label={
            <FormattedMessage
              description="ZGW-APIs registration options 'partnerRoltype' label"
              defaultMessage="Partner roltype"
            />
          }
          helpText={
            <FormattedMessage
              description="ZGW-APIs registration options 'medewerkerRoltype' help text"
              defaultMessage={`Description (omschrijving) of the ROLTYPE to use for
          citizens filling in a form with a partner.`}
            />
          }
          noManageChildProps
        >
          <PartnerRoltypeSelect
            roltypes={roleTypes}
            loading={loading}
            zgwApiGroup={zgwApiGroup}
            caseTypeIdentification={caseTypeIdentification}
