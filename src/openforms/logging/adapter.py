...
def from_structlog(event_dict: EventDict) -> EventDetails:
    ...
    match event_dict:
        ...
        case {
            ...
            | "payment_register_failure" as event,
            "submission_uuid": str(submission_uuid),
            "plugin": PaymentBasePlugin() as plugin,
        ...
        #
        case {
            "event": "stuf_zds_request"
            | "stuf_zds_success_response"
            | "stuf_zds_failure_response"
            | "stuf_bg_request"
            | "stuf_bg_response" as event,
            "service_id": int(service_id),
            "url": str(url),
        }:
            service = StufService.objects.get(id=service_id)
            return EventDetails(
                event=event,
                instance=service,
                extra_data={"url": url},
