...
class ZGWBackendVCRTests(OFVCRMixin, TestCase):
    ...
    # TODO-4877: this could be combined with the previous one by adding an additional
    #  entry to ``side_effect`` with the same message ID. Or perhaps even completely
    #  remove it?
    @patch(
        "openforms.registrations.contrib.zgw_apis.plugin.get_last_confirmation_email",
    ...
    def test_updating_registration_raises_when_confirmation_email_was_not_sent(
        self, mock_get_last_email
    ):
        submission = SubmissionFactory.from_components(
            [
