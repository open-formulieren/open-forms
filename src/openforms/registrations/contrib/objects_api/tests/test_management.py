...
class MigrateObjectsAPIObjecttypesURLSTest(OFVCRMixin, TestCase):
    ...
    def test_existing_objecttype(self):
        FormRegistrationBackendFactory.create(
            backend="objects_api",
            options={"objects_api_group": self.group.pk, "objecttype_url": "http://localhost:8001/api/v2/objecttypes/8e46e0a5-b1b4-449b-b9e9-fa3cea655f48"}
        )
        ...
        self.assertEqual(
            backend.options["objecttype_name"], "Person"
        )

    def test_objecttype_does_not_exist(self):
        FormRegistrationBackendFactory.create(
            backend="objects_api",
            options={"objects_api_group": self.group.pk, "objecttype_url": "http://localhost:8001/api/v2/objecttypes/123"}
        )
        ...
        self.assertEqual(
            backend.options["objecttype_url"], "http://localhost:8001/api/v2/objecttypes/123"
        )
