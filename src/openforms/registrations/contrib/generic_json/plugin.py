...
# TODO-5312: the schema processing can be completely removed here if we
#  1) Have the plugin options available in `generate_json_schema`
#  2) Utilize the submission in `generate_json_schema` to determine the number of
#     attachments.
#  OR if we are OK with the schema being a bit ambiguous for file and selectboxes
#  components:
#   - File components with multiple false having `"type": ["null", "object"]`
#   - Selectboxes components having a `"oneOf": [object_schema, list_schema]`
def process_component(
    ...
    match component:
        ...
        case {"type": "file"}:  # multiple is False or missing
            ...
            if n_attachments == 0:
                ...
                for key_to_remove in ("properties", "required", "additionalProperties"):
                    schema["properties"][key].pop(key_to_remove)
            else:
        ...
        case {"type": "selectboxes"} if key in transform_to_list:
            ...
            # The schema representing a list is the second entry of the oneOf
            schema["properties"][key].update(schema["properties"][key]["oneOf"][1])
            del schema["properties"][key]["oneOf"]

        case {"type": "selectboxes"} if "oneOf" in schema["properties"][key]:
            # If there is no oneOf key, it means the schema wasn't processed previously.
            # The schema representing an object is the first entry of the oneOf
            schema["properties"][key].update(schema["properties"][key]["oneOf"][0])
            del schema["properties"][key]["oneOf"]
