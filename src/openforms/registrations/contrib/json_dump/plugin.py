...
class JSONDumpRegistration(BasePlugin):
    ...
    def process_variables(submission: Submission, values: JSONObject):
        ...
        for key in values.keys():
            ...
            # Note: we check the length of the array instead of making the decision
            # based on component["multiple"], because the number of attachments can
            # still be one when the component supports multiple uploads.
            match len(encoded_attachments):
                case 0:
                    values[key] = {}
                case 1:
