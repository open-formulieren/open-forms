...
def execute_component_pre_registration(
    ...
    # TODO: make sure the whole formio configuration wrapper operates on msgspec
    # structs instead, and use a custom celery serializer maybe?
    _component = _convert_legacy_component(component)

    component_key = _component.key
    submission = Submission.objects.get(id=submission_id)
    ...
    try:
        result = formio_registry.apply_pre_registration_hook(_component, submission)
    except Exception as exc:
