...
# TODO-5696: probably good to just raise all KeyErrors, then catch and log them, instead
#  of falling back to defaults silently? Though, non of the field are listed as required
#  in the schema


def with_graceful_default[T, **P](default: T):
...
class JccRestPlugin(BasePlugin):
    ...
    def get_available_products(
        ...
        products = [
            Product(
                ...
                description=product.get("necessities") or "",
            )
            for product in data
        ]
    ...
    def _create_location(location: RawLocation) -> Location:
        ...
        # data include an empty string instead of None, so we include it in the checks
    ...
    def get_times(
        ...
        return client.get_available_times_for_appointment(
            location.identifier,
            day,
