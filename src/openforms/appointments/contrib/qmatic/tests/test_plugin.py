...
class PluginFailTests(TestCase):
    maxDiff = 1024
    ...
    @requests_mock.Mocker()
    def test_get_available_products_http400(self, m):
        m.get(
            f"{self.api_root}services",
            text=mock_response("error.json"),
            status_code=400,
            headers={"ERROR_CODE": "142", "ERROR_MESSAGE": "Service not found"},
        )
    ...
    @requests_mock.Mocker()
    def test_get_available_products_http500(self, m):
        m.get(
            f"{self.api_root}services",
            text=mock_response("error.json"),
            status_code=500,
    ...
    @requests_mock.Mocker()
    def test_get_available_products_error(self, m):
        m.get(
            f"{self.api_root}services",
            text=mock_response("error.json"),
            status_code=200,
