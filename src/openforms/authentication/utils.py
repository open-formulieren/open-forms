...
def get_authentication_plugin(request: Request, form: Form) -> str:
    try:
        ...
        raise KeyError("Plugin cannot be retrieved from session")
...
def is_authenticated_with_an_allowed_plugin(request: Request, form: Form) -> bool:
    try:
        ...
        return form.auth_backends.filter(backend=plugin_id).exists()
    except KeyError:
        return False
...
def meets_plugin_requirements(request: Request, form: Form, plugin_id: str) -> bool:
    ...
    return plugin.check_requirements(request, plugin_configuration.options or {})
