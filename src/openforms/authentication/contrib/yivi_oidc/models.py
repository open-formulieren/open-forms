...
class YiviOpenIDConnectConfig(AuthorizeeMixin, OpenIDConnectConfigBase):
    ...
    bsn_scope = models.CharField(
        _("bsn scope"),
        max_length=100,
        default="bsn",
        help_text=_(
            "The scope used during BSN authentication requests to fetch the BSN claim."
        ),
    )
    ...
    kvk_scope = models.CharField(
        _("kvk scope"),
        max_length=100,
        default="kvk",
        help_text=_(
            "The scope used during KVK authentication requests to fetch the KVK claims."
        ),
    )
...
class AvailableScope(models.Model):
    config = models.ForeignKey(
        ...
        verbose_name=_("available scopes"),
    ...
    claims = ArrayField(
        ...
        blank=True,
        help_text=_(
            "List of claims that will derive from the scope. Used to correctly fetch "
            "the claims after authentication."
        ),
    )
