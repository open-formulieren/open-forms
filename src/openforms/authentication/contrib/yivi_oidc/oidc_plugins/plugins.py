...
from ...digid_eherkenning_oidc.oidc_plugins.views import anon_user_callback_view
...
class YiviPlugin(BaseOIDCPlugin, AnonymousUserOIDCPluginProtocol):
    ...
    def get_claim_processing_instructions(
        ...
        bsn_claim_path = config.options["identity_settings"]["bsn_loa_claim_path"]
        kvk_claim_path = config.options["identity_settings"]["kvk_loa_claim_path"]
        ...
        match (has_bsn_claim, has_kvk_claim):
            case True, _:
                claim_processing_instruction["optional_claims"].extend(
                    [
                        config.options["identity_settings"]["bsn_loa_claim_path"],
                        config.options["identity_settings"]["bsn_default_loa"],
                        config.options["identity_settings"]["bsn_loa_value_mapping"],
                    ]
                )
            case False, True:
                claim_processing_instruction["optional_claims"].extend(
                    [
                        config.options["identity_settings"]["kvk_loa_claim_path"],
                        config.options["identity_settings"]["kvk_default_loa"],
                        config.options["identity_settings"]["kvk_loa_value_mapping"],
                    ]
                )
            case False, False:
                pass
