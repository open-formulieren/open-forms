...
class YiviOptionsSerializer(JsonSchemaSerializerMixin, serializers.Serializer):
    authentication_options = serializers.ListField(
        ...
        help_text=_(
            "Available authentication options that the user can choice from. "
            "The user must chose one of the options. "
            "If no options are specified, a pseudo value will be used as identifier."
        ),
        default=[],
        required=False,
        allow_empty=True,
    ...
    additional_attributes_groups = serializers.ListField(
        ...
        help_text=_(
            "Additional attributes groups to use for authentication. "
            "The user can choice per group whether they provide these attributes or not."
        ),
        default=[],
        required=False,
        allow_empty=True,
    ...
    def _handle_before_import(self, data) -> None:
        ...
        valid_choices = AttributeGroup.objects.all().values_list(
    ...
    def get_fields(self):
        ...
        ].child.choices = AttributeGroup.objects.all().values_list(
            "name", "description"
        )
        return fields
