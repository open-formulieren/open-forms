...
class OFEIDASConfig(BaseConfig):
    class Meta:
        ...
        verbose_name_plural = _("eIDAS (OIDC)")
    ...
    person_identifier_type_claim = ClaimField(
        ...
        help_text=_(
            ...
            "The expected claim value is one of: 'bsn', 'pseudo' or 'national id'."
        ),
    )
    ...
    # Company claims are not returned by the Signicat Simulator, and as we currently
    # don't have a test environment, we cannot guarantee these values are correct.
    ...
    company_identifier_type_claim = ClaimField(
        verbose_name=_("company identifier type claim"),
        default=ClaimFieldDefault("namequalifier"),
        help_text=_(
            "Claim that specifies how the company identifier claim must be interpreted. "
            "The expected claim value is one of: 'kvk', 'rsin', 'vat', 'lei', 'eori', "
            "'ntr' or 'opaque'."
        ),
    )
    ...
    CLAIMS_CONFIGURATION = (
        ...
        {"field": "company_identifier_type_claim", "required": False},
