...
class AzureADOIDCAuthPluginEndpointTests(APITestCase):
    ...
    def test_plugin_list_azure_ad_oidc_enabled(self, mock_get_solo):
        mock_get_solo.return_value = GlobalConfiguration(
            plugin_configuration={
                "authentication": {
                    "azure_ad_oidc": {"enabled": True},
                },
            }
        )
        ...
        self.assertIn("azure_ad_oidc", plugin_names)

    @patch("openforms.plugins.registry.GlobalConfiguration.get_solo")
    def test_plugin_list_azure_ad_oidc_not_enabled(self, mock_get_solo):
        mock_get_solo.return_value = GlobalConfiguration(
            plugin_configuration={
                "authentication": {
                    "azure_ad_oidc": {"enabled": False},
                },
            }
        )
        ...
        plugin_names = [p["id"] for p in response.data]
        self.assertNotIn("azure_ad_oidc", plugin_names)
