...
from openforms.authentication.contrib.generic_oidc.plugin import get_config_to_plugin
...
class OFOIDCBackend:
    def extract_and_store_claims(self, claims: JSONObject, request) -> None:
        raise NotImplementedError()

    def process_claims(self, claims: JSONObject, config: BaseConfig) -> JSONObject:
        raise NotImplementedError()
...
class GenericOIDCBackend(OIDCAuthenticationBackend):
    ...
    def _check_candidate_backend(self) -> bool:
        ...
        if isinstance(self.config_class, OFOpenIDConnectConfig):
