...
class SubmissionReCaptchaSerializer(serializers.Serializer):
    token = serializers.CharField(
        help_text=_(
            "The encrypted token sent by reCAPTCHA Enterprise when the user performs an action"
        ),
        required=True,
    )
    action = serializers.CharField(
        help_text=_("The user action for which the reCAPTCHA token was generated."),
        required=True,
    )


class ReCaptchaTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if not attrs["valid"]:
            error_msg = _("The token is invalid: %(reason)s") % {
                "reason": attrs["invalid_reason"]
            }
            raise serializers.ValidationError(error_msg)
        return attrs


class ReCaptchaEventSerializer(serializers.Serializer):
    token = serializers.CharField()
    site_key = serializers.CharField()
    expected_action = serializers.CharField()


class AssessmentReCaptchaSerializer(serializers.Serializer):
    name = serializers.CharField()
    token_properties = ReCaptchaTokenSerializer()
    score = serializers.FloatField(min_value=0.0, max_value=1.0)
    event = ReCaptchaEventSerializer()

    def validate(self, attrs):
        if attrs["event"]["expected_action"] != attrs["token_properties"]["action"]:
            raise serializers.ValidationError(_("Invalid action."))
        return attrs


class AssessmentReCaptchaOutcomeSerializer(serializers.Serializer):
    allow_submission = serializers.BooleanField(
        help_text=_("Whether the user passed the reCAPTCHA assessment.")
    )


@dataclass
class AssessmentReCaptchaOutcome:
