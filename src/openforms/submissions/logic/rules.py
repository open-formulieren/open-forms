...
def get_rules_to_evaluate_v2(
    ...
    form_step = current_step.form_step

    if form_step.logic_rules.count() == 0:
        return get_rules_to_evaluate(submission, current_step)

    # Note that this an OrderedModelQuerySet, so the logic order is already resolved
    # here
    return form_step.logic_rules.iterator()
