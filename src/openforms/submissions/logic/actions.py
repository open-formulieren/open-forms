...
from typing import Any, Literal, Self, TypedDict, assert_never
...
class PropertyAction(ActionOperation):
    ...
    def apply(self, step: SubmissionStep, config: FormioConfig) -> None:
        ...
        # spell out the possible combinations so we can rely on the type checker for
        # unusual situations
        match (self.property_name, component):
            # HIDDEN
            case ("hidden", SoftRequiredErrors()):
                logger.warning(
                    "misconfigured_logic_action_detected",
            ...
            case ("validate.required", _):
                assert isinstance(self.value, bool)
                component.validate.required = self.value
            ...
            case ("disabled", _):
                assert isinstance(self.value, bool)
                component.disabled = self.value

            case _:  # pragma: no cover
                assert_never((self.property_name, component))
...
class StepNotApplicableAction(ActionOperation):
    ...
    def unresolved_output_variables(self) -> set[str]:
        ...
        return {component.key for component in formio_config}
