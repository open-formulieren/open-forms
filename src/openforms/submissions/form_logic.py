...
def _get_initial_data_for_clear_on_hide(step: SubmissionStep) -> FormioData:
    ...
    The new renderer does not only assign the empty value when a component goes from
    hidden -> visible with clearOnHide enabled. The following value will be assigned,
    in order of precedence:
      1. Already submitted value
      2. Default value
      3. Empty value
    We need to do the same in the backend, to ensure no infinite loops will occur.
    ...
    # layout components, as all the relevant fields will have a variable.
    for key, variable in state.get_variables_in_submission_step(step).items():
        if (component := config_wrapper.component_map.get(key)) is None:
            continue
