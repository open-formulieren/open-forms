...
def generate_submission_reference(
    submission_id: int, post_registration: bool = False
) -> None:
    """
    Set the submission reference if the reference is not generated by the registration backend or if there is
    ...
    backend_generates_reference = (
        backend and registry[backend].backend_generates_reference
    )

    # Setting the reference AFTER registration only if the registration backend generates it
    if post_registration and backend_generates_reference:
        set_submission_reference(submission)
        return

    # Setting the reference BEFORE registration if the registration backend doesn't generate it OR if there is no
    # registration backend
    if not post_registration and not backend_generates_reference:
        set_submission_reference(submission)
        return
