    ...
    for rule in rules:
        ...
        for key in rule.input_variable_keys:
            ...
            for dep_rule in dep_rules:
                ...
                #  should append the variable key to "variable_list" on the edge data
                #  instead.
...
def detect_cycles(graph: DiGraph) -> list[Cycle] | None:
    ...
    for rules in simple_cycles(graph):
        ...
        variables = [
            graph.get_edge_data(r1, r2)["variable"]
...
def add_missing_steps(graph: DiGraph):
    """
    Iterate over all nodes on the graph and resolve a step for logic rules that do not
    ...
    for rule in graph.nodes:
        ...
        # TODO-2409: this assumes the parents DO have a step, so it can only work if we
        #  iterate through the graph layer by layer (breadth-first search).
        # TODO-2409: what if it has no predecessors? I guess it doesn't matter on which
        #  step we execute it then :shrug:?
        parent_steps = set()
