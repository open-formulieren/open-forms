...
class DependencyGraphTests(TestCase):
    def test_create(self):
        ...
        graph = create_graph(form.formlogic_set.all())
    ...
    def test_resolve_order_with_step(self):
        form = FormFactory.create()
        FormStepFactory.create(
            form=form,
            form_definition__configuration={
        ...
        with self.subTest("Rule order"):
            self.assertEqual(resolve_order(graph), [rule_1, rule_2])
        ...
        with self.subTest("Rule order with step"):
            ...
            self.assertEqual(resolve_order_with_step(graph), [rule_2, rule_1])

    def test_add_missing_steps(self):
        """
        Ensure that rules for which we were not able to resolve the step independently,
        get assigned a step through their direct parents.

        Relevant routine to test: `add_missing_steps`.
        """

    def test_add_missing_steps_with_multiple_rules_without_step(self):
        """
        Ensure that rules for which we were not able to resolve the step independently,
        and for which their parents also have no step assigned to them, get assigned a
        step through all their parents.
