...
class DependencyGraphTests(TestCase):
    ...
    def test_no_dependencies_keeps_rule_order(self):
        # TODO-2409: why is this important?
        form = FormFactory.create(
            generate_minimal_setup=True,
            formstep__form_definition__configuration={
                "components": [
    ...
    def test_add_missing_steps(self):
        ...
        get assigned a step through their parents.
        """
        form = FormFactory.create()
        FormStepFactory.create(
            form=form,
        ...
        with self.subTest("Graph structure"):
            self.assertEqual(list(graph.nodes), [rule_1, rule_2, rule_3])
            self.assertEqual(list(graph.edges), [(rule_1, rule_2), (rule_2, rule_3)])

        add_missing_steps(graph)
        with self.subTest("Step is added"):
            # All the rules depend (directly or indirectly) on fields "foo" and "bar",
            # so step 2 is assigned to ensure we have all data available.
            self.assertEqual(rule_1.steps, {step_2})
