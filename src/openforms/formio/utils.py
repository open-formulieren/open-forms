...
def get_branch_representation(
    branch: Sequence[AnyComponent], *, prefix: str = ""
) -> str:
    """
    ...
    def _node_repr(node: AnyComponent) -> str:
        match node:
            case Columns():
                return node.key
            case _:
                return node.label or node.key

    bits = itertools.chain(
        [prefix] if prefix else [],
        (_node_repr(node) for node in branch),
    )
    return " > ".join(bits)


def get_component_default_value(component: AnyComponent | Component) -> Any | None:
    ...
    if isinstance(component, dict):
        ...
        if getattr(component, "multiple", False) and default_value is None:
            return []
