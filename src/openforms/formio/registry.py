...
class ComponentRegistry(BaseRegistry[BasePlugin]):
    ...
    def localize_component(
        ...
        # translate FAQItems
        # FIXME: remove the type-specific checks here and defer to components
        # themselves?
        match component:
            case Content() | Fieldset() | Columns() | SoftRequiredErrors() | CosignV1():
                pass
            case _:
                for faq_item in component.faq_items:
                    if (
                        enabled
                        and (meta := faq_item.open_forms) is not None
                        and (generic_translations := meta.translations) is not None
                        and (translations := generic_translations.get(language_code))
                        is not None
                    ):
                        for prop, translation in translations.items():
                            if not translation:
                                continue
                            setattr(faq_item, prop, translation)

                    # always drop translation meta information
                    if faq_item.open_forms and faq_item.open_forms.translations:
                        faq_item.open_forms.translations = None
