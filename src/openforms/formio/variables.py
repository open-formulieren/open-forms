...
from formio_types import AnyComponent, get_template_trace_context
...
def extract_variables_from_template_properties(component: AnyComponent) -> set[str]:
    """
    Extract all variables used in the template expressions of the relevant properties.

    Each component type knows which properties can be templated, so we defer to the
    ...
    def _extract_variables(source: str) -> None:
        nonlocal variables
        variables.update(extract_variables_used(source))

    component.test_templates_with_trace(do_parse=_extract_variables)
