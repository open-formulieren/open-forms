...
class PartnerListField(serializers.Field):
    ...
    def validate_list(self, partners):
        ...
        if fm_immutable_variable:
            # we do not send these information from the frontend (since they are not used
            # until for now) so we have to exclude them from the data that needs validation
            initial_value = [
                {
                    key: (
