...
Accessing the values stored in the ``SubmissionValueVariables`` should done through the
...
Note that the data is still stored as JSON in the database, and also exists our Python-type domain once we serialize
