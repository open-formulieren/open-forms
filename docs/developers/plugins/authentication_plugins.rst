...
authentication options are available during login with Yivi and which additional scopes
should be requested.
...
scopes to accept. This results in specific claims being returned.
...
Additional scopes using condiscon
---------------------------------

Due to the flexibility of available authentication options and additional scopes, we
...
``authentication_options`` and ``additional_scopes``). In the ``start_login()`` we pass
the plugin options to the Yivi AuthenticationInitView. From here, we used in the
``get_extra_params()`` function to add the scopes to the authentication request.

By passing the scopes as `condiscon <https://irma.app/docs/condiscon/>`_ we can give
users the ability to choose which scopes to accept (otherwise, all scopes would be
required). In ``_yivi_condiscon_scope()`` we create the condiscon scope and return the
...
we check against the plugin options specified ``additional_scopes``. If the claim is
derived from one of the scopes, then we add it to the ``FormAuth`` under the collective
