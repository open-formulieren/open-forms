    ...
    if operator in ["==", "!=", "===", "!=="]:
        ...
        match a, b:
            ...
            case _:
                ...
                # 1. Variable expression + other expression/data
                # 2. Two other expressions/data
                pass
    elif operator == "in":
        a, b = argument
    ...
    for form in forms_to_check.iterator(chunk_size=10):
        ...
        for form_step in form_steps:
            for component in iter_components(
                ...
                # TODO: should this include children of editgrids? Until 3.5, children
                #  of editgrids already had the new behavior, but empty editgrid data
                #  now also includes empty values for components that were hidden...
        ...
        for rule in form.formlogic_set.iterator():
            for var_name, comp_value in iter_variables_with_compare_value(
                ...
                # Visibility of component is not affected, so it's not relevant
                if resolved_key not in components_with_affected_visibility:
                    continue
                ...
                # TODO: should we also check comparison to default value? Unlikely
                #  probably, because clearing a value to the default value is only
                #  changed in 3.5. So it would only affect forms that have been built
                #  between 3.5 and 4.0, AND rely on this undocumented behavior.
                if comp_value is None or comp_value == empty_value:
